...
struct ComponentTextParamGuard {
    ...
    func publicInitTextParamsAreClassified() throws {
        ...
        #expect(result.localizedByType.count == 23,
                """
                LSK/LSR 由类型判定的键实测 23 条（`#373` 新增 FormField 的 label / description 后由 17 变为 19；`#376` Banner 便利 init 的 title / message 使 19 变为 21；`#420` TimelineItem 的 title / description 使 21 变为 23），实际 \(result.localizedByType.count)：\(result.localizedByType)。\
