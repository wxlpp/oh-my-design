...
struct ComponentRegistryGuard {
    ...
    func registrySchemaIsValid() throws {
        ...
        #expect(entries.filter { $0.repo == "ohmydesign" }.count == 71,
                "OhMyDesign 侧条目数不是 71（`#270` / `#279` 扩扫描根到 Effects / Charts / Shaders 并按判定法补录；`#380` 新增 TagGroup 后由 56 变为 57，`#422` 新增 Tree 后变为 58，`#420` TimelineItem 成为公开 View 后变为 59，`#417` StatefulButton、`#418` SlideToConfirm 各 +1；与 shaders epic 合并后为 67，`#282` 批 A 四个 paper 移植背景使 67 变为 71）——若为新增属预期变化请同步改这个数字；若无源码变更条目却变了，是静默删条目/改 repo 的信号")
