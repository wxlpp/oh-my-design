...
- **搜索过滤与命中高亮**：按关键词过滤，自动展开到命中节点并高亮匹配片段。
  ...
  **组件不自动播报**（本 epic 范围外）。
  ...
  · **性能**（`#441` 定案）：**支持规模 = 万级节点**（以 11 110 节点的均衡树与 1 × 10 000 的宽树两个夹具为准）；
  **预算 = 搜索期间「数据与查询都没变」的 body 重算（方向键移焦点、选中变化、展开 / 折叠、宿主改 `checked`）
  在 release 构建下 ≤ 8 ms（半帧）**；查询变化引起的重算允许超过一帧，读数写进 `docs/components/tree.md`「搜索性能」。
  达成方式是调用方显式给版本号：`searchFilter(_:text:version:)` 按（去首尾空白后的搜索词, 版本号, `id` / `children` key path）缓存过滤结果
  （只按搜索词做键会在数据变化时给出过期的行）；不给版本号时每次重算都遍历整树（`#423` 行为不变）。
...
- **Tree**：能以「默认展开到第 2 层」启动，展开态可被外部读写；FR-2 行为真值表七行各有判据；
  ...
  搜索性能按 `#441` 的预算：带版本号时「数据与查询都没变」的重算 ≤ 8 ms（release，万级节点夹具），缓存命中 / 失效有判据。
...
- Tree 搜索的**组件内**结果数播报、空态视图与输入法组字识别（`#423`）：三者由宿主负责，组件提供
  `Tree.searchMatches` 作接入点。搜索性能的数值预算已由 `#441` 定案（见 FR-2「性能」）。
