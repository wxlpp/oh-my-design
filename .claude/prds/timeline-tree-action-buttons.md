...
- **密度与外观配置**（`#429` 修订）：读环境 `controlSize` 推导行距 / 展开槽宽 / 缩进 / chevron / 复选框字形
  （推导表见 `docs/superpowers/specs/2026-09-23-tree-style-design.md` §1.2，iOS 行距保底 44）；
  `TreeStyle` **封闭配置**（`.automatic` / `.navigator`，经 `View.treeStyle(_:)` 注入）——**不是协议**，
  升协议的兼容路径（modifier 取 `any TreeStyle`）见同一 spec §2.1；整行右键菜单（`rowContextMenu`，
  目标集合 = 选中 ∩ 可见，或只是右键那一行）。行为（键盘、选择归约、三态勾选、焦点、无障碍、命中区）
  留在组件内，外观只决定画法。
