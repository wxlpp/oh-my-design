...
⚠️ **以下两条按实现期实测更正**（原写「唯一标签（默认 `allowDuplicates: false`）⇒ 身份完全稳定」
与「同值的后续出现会降序号」，两句都不成立）：

- 身份稳定的前提是**值在输入数组里唯一**，**不是** `allowDuplicates: false`
  ——后者只约束**提交路径**，外部绑定照样可以写进重复值（`tags = ["a","a"]`）。
  值唯一时，删中间项消失的身份**正是被点的那一项**，无新增、无任何身份换值。
- 重复标签 ⇒ **没有「降序号」这回事**。对 `["a","b","a","c","a"]`，删下标 0 / 2 / 4 三种情形的
  差集**恒为** `lost = {(a, 2)}`、`gained = {}`：排在前面的同值身份被**原样复用**，
  消失的恒是**末次出现**。⇒ 退场动画播在该值的最后一个 chip 上，不一定是用户点的那一个
  （「播在哪一项」是由差集**推出**的，没有单独量像素）。
  这一格的限制写进 `docs/components/tag-input.md`、`BREAKING-CHANGES` 与 registry 的 `TagInput.notes`。
...
2. 重复标签得到 `(值, 序号)` 互异身份；
3. **删中间项：存活项身份逐项不变**（⚠️ 原写「旧 offset 身份会让其后每一项换身份」，
   实测不是那样：下标身份下**消失的恒是末位 id**，而下标 ≥ 删除位置的每个 id 被
   **重新绑定到邻居的值**——实测 `{1: enhancement→docs, 2: docs→chore}`，即 chip 原地
   把文案换成邻居的。把实现改回 `enumerated().offset` 当场判红）；
4. 删重复项：差集恒为 `{(值, 末次序号)}`、无新增，前面的同值身份被复用
   （如实钉住限制，不假装没有）；
