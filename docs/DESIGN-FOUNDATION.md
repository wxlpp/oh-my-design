...
| `StatefulButton` 四态切换（#417） | 配件槽出现 / 消失使按钮变宽，走 `CoreMotionToken.press.transformAnimation(for:)`；loading 为匀速旋转的弧线，结果符号放大淡入，failure 另让按钮左右抖一下 | 驱动曲线为 `nil` ⇒ 宽度**直接跳到位**（同 #408 的定案：宽度补间等于横向位移）；loading 画静态循环箭头，各层直接换图，不抖动 |
