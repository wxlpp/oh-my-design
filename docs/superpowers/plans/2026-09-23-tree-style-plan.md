...
```bash
grep -nE '^(@MainActor )?struct [A-Za-z]+Tests' Tests/OhMyDesignTests/TreeTests.swift Tests/OhMyDesignTests/TouchTargetTests.swift Tests/OhMyDesignTests/CheckBoxMixedTests.swift
```

`805f40f` 上读到的 Tree suite：`TreeFlattenTests` / `TreeTruthTableTests` / `TreeKeyboardTests` /
`TreeNestedStyleTests` / `TreeAccessibilityTests` / `TreeMotionTests` / `TreeInteractionReducerTests` /
`TreeLazinessTests` / `TreeHostedWiringTests`（`#if os(macOS)`）/ `TreeRenderTests`。
⚠️ `TreeTests` 是**文件名不是类型名**；`TouchTargetTests` 整个在 `#if os(iOS)` 里（macOS 上零条）。

### 验证口径（分层）

1. **迭代**：`swift test --filter 'OhMyDesignTests\.Tree'`（命中全部 `Tree*` suite）+ 本 PR 相关的其他类型名
   （如 `--filter CheckBoxMixedTests`）。每次都核输出里 `Test run with N tests` 的 **N > 0**。
...
还原用 `git stash` / 手工改回，**不用 `git checkout -- <file>`**（基线未提交时会连修复一起删）。
...
- `Sources/OhMyDesign/Components/Tree/Tree.swift`：`Tree` 读 `@Environment(\.controlSize)`，算一次 metrics 交给
  `TreeContext`；`TreeRowView` 的 `minHeight` / 缩进 / 行间距改取 metrics；`TreeDisclosureGroupStyle` 与根 `VStack`
  的 spacing 取 `metrics.rowSpacing`；`TreeDisclosureControl` 接 metrics，chevron 字号 `compactIconSize(for:)`；
  `TreeDisclosureSlot` 宽高改取 `metrics.disclosureWidth` / `metrics.rowHeight`（不再写死）。
...
- `Tests/OhMyDesignTests/CheckBoxMixedTests.swift`（或新 suite）：「未注入布局时尺寸不变」。
...
- [ ] `CheckBox` 未注入时渲染尺寸与今天相同；`CheckBoxMixedTests` 全绿。
...
- 迭代：`--filter 'OhMyDesignTests\.Tree'`、`--filter CheckBoxMixedTests`、`--filter FieldValidationControlsTests`、
  `--filter SymbolNumericMotionTests`（后两者也引 CheckBox）、`--filter CoreMotionTokenDisciplineGuard`、
  `--filter QuotedEvidenceGuard`。
- 收尾：全量 macOS；iOS `-only-testing` 逐个列：`TouchTargetTests`、`TreeRenderTests`、`TreeFlattenTests`、
  `TreeAccessibilityTests`、`CheckBoxMixedTests` + 本 PR 新 suite；xcresult 顶层计数非零且吻合。
...
- [ ] **升协议兼容性编译探针**（spec §2.1 / R4）：scratch 下两个模块版本——v1 `public struct TreeStyle` +
  `nonisolated static var navigator` + `func treeStyle(_: TreeStyle)`；v2 `public protocol TreeStyle` +
  `extension TreeStyle where Self == NavigatorTreeStyle { static var navigator }` + `func treeStyle(_: some TreeStyle)`。
  同一份调用方代码分别编译：`.treeStyle(.navigator)`（预期两版都过）、`let s: TreeStyle = .navigator`、
  `TreeStyle.navigator`（预期 v2 不过）。读数写进 PR 正文与 spec §2.1（把「推断」改为「实测」）。
  **若 `.treeStyle(.navigator)` 在 v2 不过 ⇒ 停，回 spec 重议 D2。**
- [ ] 确认 `@Entry` 默认值是否要求 `TreeStyle: Sendable`（以编译器为准；要求则加并在 spec 登记）。
...
- 文档与登记：
  - `docs/component-registry.json` `Tree.notes` 追加 spec §6.2 的三点（封闭配置非扩展点；《D2 的边界》核对结果；
    升协议兼容路径）。`kind` / `decidedBy` / `needsExtensionPoint` 不动；`ComponentExtensionPointGuard` 不动。
...
- [ ] `.automatic` 在 `.regular` 下与 PR 1 合入态逐像素一致（spec §7.2；基准在本 PR 内先渲存 scratch，不入库）；既有 `TreeRenderTests` 全绿。
- [ ] `TreeHostedWiringTests` 对 `[.automatic, .navigator]` 参数化：按键写回、点 chevron 展开、点复选框勾选、点行选中，绑定终态逐项相等（spec §7.3）。
- [ ] 点**缩进区**选中该行，两种外观都成立。
...
- [ ] 悬停切换不带动画——源码层（`value:` 不引用 `isHovered`；`.onHover` 闭包内无 `withAnimation` / `withTransaction`）。
- [ ] 悬停状态在行级（容器无 hover 存储属性；`.onHover` 只在行宿主）。
- [ ] 悬停不重算整棵树（`content` 调用计数只有该行增加）。
- [ ] 参考线对齐 chevron 中心（位图，≤ 1pt，`.small` / `.regular`）；跨行连续；根数 = `level - 1`；RTL 镜像（spec §7.5）。
- [ ] `onHover` 真接线：试合成 `mouseMoved`；不成则登记真 HID 项（写进 tree.md「不在 CI」清单）。
...
- [ ] 悬停状态上提为容器 `@State hoveredID` → 「行级」「不重算整棵树」两条红。
...
- `Tree.swift`：`private var rowMenu: ((Set<ID>) -> AnyView)?`；`public func rowContextMenu<M: View>(@ViewBuilder _ menu: @escaping (Set<ID>) -> M) -> Tree`
  ...
  容器 body 里建一次 `visibleIDs = Set(rows.map(\.id))` 传给行宿主；行宿主 `if let menu` 才挂 `.contextMenu`。
  `Tree` 仍三个泛型。
...
- [ ] 接线：每个已构建行各收到一次正确目标集合（builder 实参捕获）。
- [ ] 未调用 `rowContextMenu` 时 builder 捕获计数 0 且不挂菜单（结构探针实现期定，写进 PR 正文）。
...
- [ ] `ForEach` 的 id 改用下标 → 展开后选中行错位（`Legacy422` 闸门「选中第 3 层行」格红）。
