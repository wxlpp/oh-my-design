...
`min(8, 16 − 内容高)`（0–8pt）；`.alternate` 中按槽宽排版后仍宽于槽的固定宽内容改为向外溢出（§3.4；文字换行不变）；`content:` 里并列的多个视图由
并排（旧 `HStack` 展平）改为竖排（§1.3 单一容器，PR 1 起生效）；空节点（`node:` 闭包什么都不产出）保留 24pt 空盒、多视图节点叠在同一个盒里
（旧实现前者整格消失、内容左移，后者拆成并排的多个 24pt 格——都是旧实现的缺陷，闸门里以「旧实现 + 24pt 空盒 / + `ZStack`」为对照）；
