...
### 6.7 PR 4 实测与落地差异（2026-09-24，macOS 托管窗口）

- **E4-0 挂载窗口**：(a) 无滚动宿主、(b) `ScrollView` 首屏行、(d) `ImageRenderer` 的首次 `true` 回调都落在窗口开着时，(c) 挂载后滚入时窗口已关——
  前提是关窗用 `RunLoop.main.perform(inModes: [.common])`；`DispatchQueue.main.async` 在单测进程里整个测试期间都不执行（MainActor 作业占着主队列），
  按它写的窗口在测试里永不关。R13 按此落地。
...
- **静止帧取整数判定**：`P` 为 `Double`，静止时（插值值 == 模型值）连线与圆点仍按 `phase(forStep:)` 判，端点 ±1 在 `Double` 里做 ⇒ §4.1 登记的溢出不再 trap，极值 `step` 的静止帧不受精度影响。
- **形态插值不换分支**：圆点三层（空心环 / 实心 / 外环）恒在、只调不透明度；连线着色层恒挂、长度 0 即不画。`if` / `switch` 换分支会在动画事务里带出淡入淡出转场，第 3 行圆点会从第 1 帧起变化（实测）。
- **入场不用 `keyframeAnimator`，也不用「插值值 ≠ 模型值」**：前者在单测进程里时钟几乎不走（0.4s 只推进约 7ms 的关键帧进度）；后者起止两端的模型输出相同（都是 1×），在飞帧不进渲染（`cacheDisplay` 全程读到终态，View 与 ViewModifier 两种写法都一样）。
  落地为**待入场态**：挂载时先收到 `false` 的行（在屏外）置为 0.86×、透明，第一次 `true` 时若窗口已关且 `.animated` 即在 `withAnimation` 里回到终态。
  ⇒ §6.3「同步 / 推迟触发」的闪帧问题不再存在（滚入前节点已是透明的待入场态），§9.5 的「trigger 推迟到下一轮 runloop」变异不再判红，改为登记；
  挂载后追加、直接出现在屏上的行不播（它不经过 `false`）。
- **RTL 横向部分着色**（PR 3 登记的缺口）：SwiftUI 对容器级 `Layout` 与其中连线的绘制整体镜像，着色层不需要 `flipsForRightToLeftLayoutDirection`（加与不加读数逐帧相同）。
- **§9.5 变异里判不红的几条**：「闩锁不看挂载窗口」——挂载时可见的行只收到 `true`、从不进入待入场态，窗口在此设计下是冗余防线；
  「入场裁决不看呈现」与「RM 下也置待入场」——各自被另一条兜住（RM 下行不进入待入场 ⇒ 不看呈现也不播；待入场后 RM 下不播 ⇒ 置了也看不到）；
  「默认待入场」——挂载时可见行首个回调即回到终态，`ImageRenderer` 仍画终态，判据判不红。以上都是**等价或近等价变异**，记录于此不再加判据。
