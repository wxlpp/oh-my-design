...
| 内容高 < 16pt 的非末行变高 | 行高由 `max(24, 内容高 + lg)` 改为 `max(盒高 + CoreSpacing.sm, 内容高 + CoreSpacing.lg)`，保证节点下方至少留 8pt 连线；内容高 ≥ 16pt 时不变。⚠️ macOS 上 `.coreFont(.callout)` 单行文字的行高不到 16pt，这类行每行多出约 1pt |
| `.alternate` 左槽内容宽于槽时向外溢出 | 原来左槽里固有宽度超过槽宽的内容向右溢出、盖住中轴上的节点；现在向左（远离中轴）溢出，节点与连线不被遮挡。右槽本来就向外溢出，不变 |
