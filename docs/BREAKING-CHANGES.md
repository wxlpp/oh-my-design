...
| `content:` 里并列的多个视图改为竖排 | 原来 `content: { Text("A"); Text("B") }` 这类并列视图被展平进行里的 `HStack`，左右并排；现在包在一个 `VStack(alignment: .leading, spacing: 0)` 里，上下竖排。要并排请自己写 `HStack` |
| 空节点 / 多视图节点 | `node:` 闭包什么都不产出（如 `if` 不成立）时，原来这一格节点整个消失、内容左移到节点列；现在保留 24pt 空盒，内容与其他行对齐。`node:` 里并列多个视图时，原来拆成并排的多个 24pt 格；现在叠在同一个节点盒里居中 |
