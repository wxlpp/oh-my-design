...
| `Text.init(verbatim:highlighting:)`（`#423`） | 以原文显示并高亮与搜索词匹配的片段（加粗 + `searchMatchBackground` 底色），与 `searchFilter` 同一个匹配函数 |
| `Color.searchMatchBackground` / `Color.systemYellow`（`#423`） | 搜索命中底色（第 3 层，系统黄 × 0.35）与它的第 2 层来源（桥接 `UIColor` / `NSColor.systemYellow`） |
...
搜索期间键盘、`Cmd/Ctrl+A`、右键菜单与焦点只作用于留下的可见行；**父行复选框只级联留下的叶后代**、三态也只反映它们
（不搜索时仍级联全部叶后代）。**下游要改什么：不用改。** 可能碰到的是**名字歧义**：下游若自己给 `Color` 扩展了
