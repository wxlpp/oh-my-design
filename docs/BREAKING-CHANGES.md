...
公开符号一个都没变，静止帧与 PR 3 逐像素相同。可见的变化只在过渡过程中：

- **阶段推进**：`progress` 变化时连线沿线生长（回退从远端收）、默认圆点在空心 / 靶心 / 实心之间插值，取 `CoreMotionToken.reveal`；
  系统「减弱动态效果」开启时连线不生长、改为整段淡入淡出。调用方自己在 `withAnimation` 里改 `progress` 时，组件用自己的曲线覆盖。
- **节点入场**：`Timeline` 挂载时在屏外的行，挂载后第一次滚入视口时缩放 0.86 → 1 并淡入；挂载时已在屏上的行、没有滚动宿主时的全部行
  不播 ⇒ 快照与 `ImageRenderer` 导出仍是终态。「减弱动态效果」下不播。挂载时在屏外的行在滚入前处于 0.86×、透明的待入场态——
  调用方若在屏外量取节点的渲染像素（例如对长时间线整体截图），会看到这些行的节点是透明的。
