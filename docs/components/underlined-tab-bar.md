...
- 下划线：`Color.accent`，厚度 `CoreBorderWidth.thick`（2pt），通过 `matchedGeometryEffect` 以 `CoreMotionToken.selection` 过渡；Reduce Motion 下不滑，旧位置淡出、新位置淡入（#407）
- 选中项滚到中间：`CoreMotionToken.selection`（`.snappy`，0.22 s；#407 前是 0.2 s snappy），Reduce Motion 下直接到位
