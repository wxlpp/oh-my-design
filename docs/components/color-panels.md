...
一组半透明彩色面板绕中轴翻转 / A set of translucent color panels flipping around a central axis，
像透视中的百叶。

源码：`Sources/OhMyDesignShaders/ColorPanels.swift`（`OhMyDesignShaders` product）。
...
| style | Style | `.regular` | 面板质感 |
| motion | ShaderMotion | `.regular` | 运动速度档位 |

## Style

语义枚举；上游的「边缘高光」开关折进本枚举，不单独暴露为 Bool：
...
```swift
ColorPanels().ignoresSafeArea()

ColorPanels(tint: .teal, style: .crisp, motion: .lively)
```

## 来源 / Provenance

移植自 paper-design/shaders 的 `packages/shaders/src/shaders/color-panels.ts` @ `43cd68d`
...
背景，**部分落地**》一节（`ColorPanels` 在「已落地的件」清单内），及 `docs/shader-provenance.md`
