...
public extension Tree {
    ...
    // 带版本号的重载（#441）：数据或文案变了必须换 version；搜索词与 version 都不变时复用上一次的过滤结果
    func searchFilter(_ query: String, text: @escaping (Data.Element) -> String, version: some Hashable) -> Tree
...
| 惰性 | **不搜索时零代价**（`TreeLazinessTests` 的承诺不变，加一条「设了 `searchFilter` 但搜索词为空」的格）；不带版本号时**搜索期间每次 body 求值遍历整树一次**，读每个节点的 `children` 与投影文案并各做一次匹配。body 求值不只由换词触发：选中变化（点行、`Space`、`Cmd+A`）、焦点移动（方向键）、展开 / 折叠、宿主改 `checked` 都会让 `Tree` 的 body 重算，每次都再遍历一遍。`#441` 起可用 `searchFilter(_:text:version:)` 让这些重算复用上一次的结果，见下方「搜索性能：版本号缓存」 | 不读折叠子树就不可能知道折叠子树里有没有命中。通用 `Data` 无法廉价判等，只按搜索词做键会在数据变化时给出过期的行，所以缓存要调用方显式给版本号 |
...
```swift
// 版本号跨数据源唯一：同一棵 Tree 先后显示不同 store 时，只用 revision 会撞键
struct StoreRevision: Hashable {
    let storeID: UUID
    let revision: Int   // 数据模型每次修改都 +1；文案投影换了也要换
}

.searchFilter(self.query, text: \.name, version: StoreRevision(storeID: self.store.id, revision: self.store.revision))
```

什么时候用：数据量大（数千节点以上）、搜索期间又有频繁的选中 / 键盘操作时用带版本号的重载；说不清数据何时变的，
用不带版本号的 `searchFilter(_:text:)`——它每次重算都重新过滤，永远不会过期。
⚠️ 宿主侧自己调 `Tree.searchMatches(...)`（算命中数、判空态）同样是整树遍历：放在宿主 body 里每次求值都算，
预算会被宿主自己吃掉——也要按（搜索词, 版本号）记忆化，或只在搜索词 / 数据变化时算一次存起来。

**预算定案**（写进 PRD FR-2「性能」）：支持规模 = **万级节点**，以均衡树（10 叉 4 层，11 110 节点）与宽树
（1 个根 × 10 000 叶子）两个夹具为准；搜索期间「数据与查询都没变」的重算——方向键移焦点、选中变化、展开 / 折叠、
宿主改 `checked`——在 release 构建下 **≤ 8 ms（半帧）**；查询变化引起的重算允许超过一帧，读数登记在下表。

| 问题 | 定案 | 理由 / 放弃的方案 |
|---|---|---|
| 缓存键 | （去首尾空白后的搜索词, 调用方给的版本号, `id` / `children` key path）；只记最近一个键 | **只按搜索词做键是错的**：数据变了会给出过期的行。运行时判等（`data as? any Equatable`）本身就是 O(n)，省不下遍历 |
| 缓存什么 | 过滤结果（命中 / 自动展开 / 保留集合）；可见行（另以生效展开集合作键，展开 / 折叠后重新展平）；父行复选框的叶后代（按行，外加一份 `Set` 供三态读数用集合运算求）。**展开态本身不缓存**：持久化集合与搜索期间的 overlay 每次照常派生 | 展开 / 折叠不改变过滤结果，只改变可见行；宽树里展平一次也要逐个子节点查保留集合，按生效展开集合判等比它便宜 |
| 不给版本号 | 行为与 `#423` 相同：每次 body 求值遍历整树 | 不破坏既有调用方；不确定数据何时变时，这是永远不会过期的那条路 |
| 不搜索时 | 只要传了 `version`，可见行与父行复选框的叶后代**不搜索时也缓存**（键里的搜索词记为空串） | 两者只取决于数据与展开态，与搜索无关；不搜索时同样省掉展平与每行一次的子树遍历 |
| 版本号失约（数据或文案变了、版本号没变） | 后果不止「显示旧数据的过滤结果与行」。**可见行缓存着元素本身**：`content` 收到的是缓存的旧 `Element`，调用方改了元素上任何字段（即使不在 `text` 投影里，如角标数、图标）而没换版本号，行内容显示旧值——只要传了 `version`，搜不搜索都一样。另外，父行复选框按**旧的叶后代**显示三态，点击时经 `TreeChecking.applying` 把旧的叶子 ID 并进 `checked`（`checked.union(leaves)` / `subtracting`）——**已删除叶子的 ID 被写进宿主的 `checked`、新增的叶子被漏掉**；搜不搜索都会发生。同一棵 Tree 先后显示不同数据源时，版本号必须跨数据源唯一 | 组件无从发现失约（判等本身就是它要省的 O(n)），所以把契约与后果写全在 `version` 的文档注释里 |
| 父行复选框的第二次遍历 | 去掉：保留的叶后代改为用保留集合筛全部叶后代（不带版本号时也生效），每行只遍历子树 **1** 次；带版本号时连这 1 次也按行缓存 | 保留集合对祖先封闭、留下的父节点至少留下一个子节点 ⇒ 筛出来的与「只沿保留节点遍历」逐项相同（`TreeCheckScopeMemoTests` 逐节点对表） |

读数由 `scripts/tree-search-benchmark/run.sh` 产出（macOS 上 400 × 600 pt 的 `NSHostingView`，`Tree` 放在 `ScrollView` 里、
多选、行内容为 `Text(verbatim:highlighting:)`；从改宿主状态计时到 `layoutSubtreeIfNeeded()` 返回，即 body 重算 + 视口内的行更新；
每格预热 3 次后取 25 次的中位数 / p95，每格每轮一个新进程，三轮取中位数；`stale` 计数为 0，即每次改动都触发了重算）。
「修前」列是用 `BASE=` 指向 `#459` 合入后的 `main`（`2888fddc`）测的。Apple M2 Pro / macOS 26.3.1，release，外接 3440 × 1440 显示器亮着，
1 分钟负载 4.5–8.7。单位 ms，中位数 / p95：

| 场景 | 修前 | 修后·不带版本号 | 修后·带版本号 | 同形对照：不搜索·带版本号 |
|---|---|---|---|---|
| 均衡树少量命中：选中视口内的行 | 21.7 / 22.6 | 21.8 / 24.4 | **4.6 / 5.7** | 3.7 / 4.6（展开 0、1、0-7、1-7） |
| 宽树少量命中：选中视口内的行 | 20.5 / 22.6 | 19.8 / 20.6 | **4.2 / 4.8** | — |
| 均衡树少量命中 + 复选框：选中视口内的行 | 25.1 / 29.6 | 25.7 / 29.3 | **5.5 / 6.4** | — |
| 宽树少量命中 + 复选框：选中视口内的行 | 26.7 / 29.7 | 24.5 / 25.4 | **5.7 / 6.9** | — |
| 均衡树少量命中：选中视口**外**的行 | 24.4 / 31.8 | 25.6 / 29.5 | 13.0 / 17.0 | 11.4 / 15.4 |
| 均衡树少量命中：宿主改 `expanded`（可见行不变） | 21.8 / 27.9 | 22.3 / 27.9 | 10.5 / 14.3 | 11.3 / 16.8 |
| 均衡树少量命中 + 复选框：宿主改 `checked` | 25.1 / 29.4 | 25.1 / 30.2 | 16.8 / 21.0 | 22.2 / 27.9 ⚠️ 不同形 |
| 宽树少量命中 + 复选框：宿主改 `checked` | 25.5 / 27.4 | 24.5 / 26.8 | 14.6 / 18.2 | — |
| 均衡树**全部命中**（11 110 行全部可见）：改选中 | 32.1 / 39.7 | 32.9 / 41.2 | 17.7 / 21.5 | 7.9 / 11.6（全部展开） |
| ↳ 同上，行内容换成不画高亮的 `Text(verbatim:)`（`ROW_CONTENT=plain`，另一会话，3 轮） | — | 28.3 / 34.7 | 8.7 / 13.1 | 5.8 / 8.5 |
| 换词：均衡树少量命中 | 25.6 / 29.6 | 23.7 / 30.0 | 24.2 / 30.9 | — |
| 换词：均衡树全部命中 | 31.3 / 35.4 | 32.8 / 42.3 | 31.8 / 35.7 | — |
| 换词：宽树少量命中 | 23.1 / 28.3 | 23.7 / 32.4 | 22.9 / 30.7 | — |
| 对照·不搜索：宽树全部展开（10 001 行），改选中 | 11.8 / 13.4 | 11.9 / 13.1 | 2.7 / 3.7 | — |

「修后·不带版本号」一列与修前持平：不给版本号时行为与代价都不变。同形对照列是**同一份修后代码、同样带版本号、不搜索**，
可见行的形状与被测格相同，用来把搜索路径的残余代价与 `Tree` 本身的更新代价分开。

- **满足预算的**：选中视口内的行（方向键移焦点、点行都属此类），四格中位数 4.2–5.7 ms，带不带复选框都在 8 ms 内。
- **超预算、与同形对照持平的**：选中视口外的行、宿主改 `expanded`。这两格带版本号后与「不搜索」的同形对照持平，
  剩下的是 `Tree` 本身的更新代价——不搜索时一样超。另开 issue 跟进，不在本 issue。
- **宿主改 `checked`：对照不同形，不下结论**。对照格走的是**不搜索**的复选框路径（父行把每个叶后代各做成一路 `Toggle`
  来源，根层 10 行就是上万路），被测格走搜索期间的 `TreeCheckBindings.scoped`（每行 2 路），两者不可比。
  实测只有：被测格带版本号 16.8 / 21.0 ms，比同会话里「选中视口内的行 + 复选框」的 5.5 ms 多约 11 ms。
  可复现的候选解释（未逐一证实）：改 `checked` 让视口内每一行的复选框来源都重新求值并重画，而改选中只动两行。
- **全部命中**：比同形对照多约 10 ms。行内容换成不画高亮的 `Text(verbatim:)` 后（表中 ↳ 一行，另一会话、3 轮取中位数）：
  全部命中 8.7 ms、对照 5.8 ms。⇒ 高亮 ≈ 17.7 − 8.7 ≈ **9.0 ms**（视口内每行都有命中片段）；搜索路径本身的残余 ≈ 8.7 − 5.8 ≈ **2.9 ms**
  （候选：生效展开集合的并集与可见行缓存的展开集合判等，均为 O(父节点数)）；对照本身也含 ≈ 7.9 − 5.8 ≈ **2.1 ms** 的空词高亮
  （`Text(verbatim:highlighting:)` 在搜索词为空时仍构造 `AttributedString`）。两格读数不在同一会话，差值只作量级。
  高亮的代价在调用方的行内容里，另开 issue 跟进。
- **换词**：键变了必须重算，与修前相同，21–32 ms，超过一帧；这是定案允许的。
- **带 `checked` 的额外代价**：选中视口内的行时多约 1 ms（均衡树 5.5 vs 4.6、宽树 5.7 vs 4.2）；修前多 3–7 ms。
- ⚠️ **绝对值随会话变化**：同一台机器、同一个二进制，另一次会话里「选中视口外的行」测得 4.4 ms（当时显示器很可能处于睡眠状态），
  本表会话是 13.0 ms。修前 / 修后与同形对照都在同一会话里交替跑，比值可信；跨会话比绝对值不可信。
- **未测**：iOS 设备、峰值内存、从输入查询到画面呈现的端到端延迟、方向键经真 HID 的读数（移焦点与改选中走同一个 body 重算）。
...
- **搜索（`#423`）**：`searchFilter` 的搜索词为空（去首尾空白后）时不遍历，上面几条不变
  ...
  ⇒ 万级节点下一次遍历就超过 60 Hz 的一帧（16.7 ms）。`#441` 起可用 `searchFilter(_:text:version:)` 让数据与查询都没变的重算
  跳过这次遍历；预算、读数与未测项见「搜索性能：版本号缓存」。
- ⚠️ 传了 `checked` 时，父行复选框的三态要读它**全部叶后代**的勾选态，因此会遍历该父行的整棵子树，
  折叠与否都一样。这是三态派生本身的代价。行宿主每次 body 求一份勾选范围（`TreeRowCheckScope`），复选框与行内容上的
  勾选动作共用：遍历子树 **1** 次（`#441` 起搜索期间也是 1 次：保留的叶后代用保留集合筛出，不再遍历第二次）；
  用带版本号的 `searchFilter` 时，这 1 次按行缓存，版本号或搜索词变了才重算。
  `⌥Space` 只在按键时对焦点行遍历一次。
...
`#441` 起的缓存判据（`TreeSearchCacheTests.swift`）：
- `TreeSearchMemoTests`（双腿，纯函数）：同一个键只算一次；换版本号、换搜索词、换 key path 都重算；不给版本号每次都算、
  不留缓存；视图用的 frame 命中缓存时不再读文案，但展开态照样跟着持久化集合走。
- `TreeCheckScopeMemoTests`（双腿，纯函数）：父行勾选范围按行与键缓存、键变即失效；留下的每一行上「用保留集合筛」与
  「只沿保留节点遍历」逐项相同；按集合求三态来源与按数组求在全部勾选组合上相同。
- `TreeSearchCacheHostedTests`（**仅 macOS**，托管窗口）：带版本号时改选中 / 展开 / 勾选、只差首尾空白的搜索词都不读文案，
  换版本号、换搜索词各重读；不带版本号时改选中照旧重读；带与不带版本号时搜索期间连点两次父行复选框的结果逐条相同。
...
`#441` 的 `searchFilter(_:text:version:)` 同样**不改判**：版本号是调用方对自己数据的声明，只决定何时重算，
匹配谓词与行的骨架、含义都不变。
