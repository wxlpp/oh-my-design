...
- **搜索（`#423`）**：`searchFilter` 的搜索词为空（去首尾空白后）时不遍历，上面几条不变
  ...
  ⇒ 万级节点下一次遍历就超过 60 Hz 的一帧（16.7 ms）。**数值预算与支持规模未定，发布前确定**（PRD 已登记）。
