...
```swift
// store.revision：数据模型每次修改都 +1（文案投影换了也要换）
.searchFilter(self.query, text: \.name, version: self.store.revision)
```
...
| 缓存什么 | 过滤结果（命中 / 自动展开 / 保留集合）与父行复选框的叶后代（按行，外加一份 `Set` 供三态读数用集合运算求）。**展开态不缓存**：持久化集合与搜索期间的 overlay 每次照常派生 | 展开 / 折叠不改变过滤结果，只改变可见行 |
...
读数（Apple M2 Pro / macOS 26.3.1，release；400 × 600 pt 的 `NSHostingView`，`Tree` 放在 `ScrollView` 里、多选、
行内容为 `Text(verbatim:highlighting:)`；计时从改宿主状态到 `layoutSubtreeIfNeeded()` 返回，即 body 重算 + 视口内的行更新；
每格预热 3 次后取 25 次的中位数 / p95，三轮各起新进程、取三轮的中位数。⚠️ 测量期间本机 1 分钟负载 50–80（其它进程在编译），
p95 偏高；修前、修后不带版本号、修后带版本号三者交替跑，彼此可比）：

| 场景 | 修前 | 修后·不带版本号 | 修后·带版本号 |
|---|---|---|---|
| 均衡树少量命中：改选中 | 21.6 / 22.3 | 21.6 / 23.8 | **4.4 / 4.9** |
| 均衡树少量命中：改持久化展开集合 | 21.6 / 25.6 | 21.6 / 24.8 | **4.2 / 6.2** |
| 均衡树少量命中 + 复选框：宿主改 `checked` | 24.6 / 28.4 | 25.1 / 27.1 | **5.6 / 7.0** |
| 均衡树少量命中 + 复选框：改选中 | 24.7 / 26.8 | 25.2 / 29.8 | **5.9 / 7.1** |
| 宽树少量命中：改选中 | 19.9 / 25.6 | 19.9 / 23.8 | **5.8 / 10.3** |
| 宽树少量命中 + 复选框：改选中 | 27.3 / 30.7 | 26.3 / 30.6 | **6.9 / 10.4** |
| 宽树少量命中 + 复选框：宿主改 `checked` | 25.7 / 28.9 | 25.0 / 26.6 | **7.1 / 8.2** |
| 均衡树**全部命中**（11 110 行全部可见）：改选中 | 30.9 / 35.8 | 30.6 / 34.1 | 15.9 / 17.6 ⚠️ |
| 换词：均衡树少量命中 | 22.6 / 24.3 | 22.7 / 26.0 | 22.9 / 27.0 |
| 换词：均衡树全部命中 | 32.0 / 34.7 | 32.5 / 35.5 | 32.0 / 34.6 |
| 换词：宽树少量命中 | 21.3 / 25.2 | 21.3 / 24.1 | 21.2 / 23.7 |
| 对照·不搜索：均衡树根层折叠，改选中 | 2.7 / 3.9 | 2.7 / 3.4 | 2.7 / 4.3 |
| 对照·不搜索：宽树全部展开（10 001 行可见），改选中 | 11.4 / 15.8 | 11.1 / 15.4 | 11.4 / 16.5 |
| 对照·不搜索 + 复选框：均衡树根层折叠，改选中 | 23.9 / 29.2 | 24.2 / 28.0 | 21.4 / 26.9 |

（单位 ms，中位数 / p95。「修后·不带版本号」一列与修前相同，证明不给版本号时没有行为或代价变化。）

- **预算：少量命中的七格中位数全部 ≤ 8 ms**，满足定案。宽树两格的 p95 超过 8 ms，出现在高负载下；中位数与其它格一致。
- ⚠️ **全部命中（整棵树都可见）不满足**：缓存后仍 15.9 ms。剩下的是 11 110 个可见行本身的展平与 `ForEach` 代价，
  与搜索无关——不搜索、宽树全部展开到 10 001 行时同样是 11.4 ms（对照行）。要压到预算内得改展平 / 行身份的通用路径，不在本 issue。
- **换词**：缓存不省（键变了必须重算），与修前相同，一次 21–32 ms，超过一帧；这是定案允许的。
- **带 `checked` 的额外代价**：带版本号时，少量命中下比不带复选框多 1.3–1.5 ms（均衡树 5.9 vs 4.4、宽树 6.9 vs 5.8）；
  修前多 3–7 ms。⚠️ **不搜索时**父行复选框的代价另有来源：不搜索的父行把每个叶后代各做成一路 `Toggle` 来源，
  均衡树根层 10 行就是 10 000 路（对照行 21–24 ms）。这条与搜索无关，本 issue 不处置。
- **未测**：iOS 设备、峰值内存、从输入查询到画面呈现的端到端延迟、方向键经真 HID 的读数（改选中与移焦点走同一个 body 重算）。
