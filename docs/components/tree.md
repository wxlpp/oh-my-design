...
| 版本号失约（数据或文案变了、版本号没变） | 后果不止「显示旧数据的过滤结果与行」：父行复选框按**旧的叶后代**显示三态，点击时经 `TreeChecking.applying` 把旧的叶子 ID 并进 `checked`（`checked.union(leaves)` / `subtracting`）——**已删除叶子的 ID 被写进宿主的 `checked`、新增的叶子被漏掉**；搜不搜索都会发生。同一棵 Tree 先后显示不同数据源时，版本号必须跨数据源唯一 | 组件无从发现失约（判等本身就是它要省的 O(n)），所以把契约与后果写全在 `version` 的文档注释里 |
...
| 均衡树少量命中 + 复选框：宿主改 `checked` | 25.1 / 29.4 | 25.1 / 30.2 | 16.8 / 21.0 | 22.2 / 27.9 |
...
- **超预算、但与搜索无关的**：选中视口外的行、宿主改 `expanded`、宿主改 `checked`。这三格带版本号后已与「不搜索」的同形对照持平
  （甚至更低），剩下的是 `Tree` 本身的更新代价——不搜索时一样超。这部分另开 issue 跟进，不在本 issue。
- **全部命中**：比同形对照（不搜索、全部展开）多约 10 ms。把行内容换成不画高亮的 `Text(verbatim:)`（`ROW_CONTENT=plain`）
  再测：全部命中 8.2–8.4 ms、对照 5.6–6.0 ms ⇒ 其中约 8–11 ms 是**逐行的 `Text(verbatim:highlighting:)`**（视口内每行都有命中片段），
  约 2.5 ms 是搜索路径本身的残余（保留集合与生效展开集合的集合运算）。高亮的代价在调用方的行内容里，另开 issue 跟进。
