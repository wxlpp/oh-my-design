...
| elevation | CoreElevation.Level | .small | 投影档位。`.none` 一行退回无投影。卡片自身有效层级为 elevated 时一律不出投影（见下） |
...
- 嵌套层级：`.surface(_:)` 按有效层级取背景（规则见 [surface.md](surface.md)）——顶层 `Card` 处在 raised、取 `surfaceCard`；放进 `.surface(.content)` / `.surface(.grouped)` / 另一个 `Card` 里时处在 elevated、取 `surfaceElevated`，再往里封顶不变。`Card` 的背景层级全部来自 `.surface`；它只额外读父层级来收起投影。iOS 上嵌套到 elevated 的 `.content` 不描边，与 `.grouped` 同观感。⚠️ macOS 上 `surfaceCard` 与 `surfaceElevated` 同值：`.content` 在 macOS 上嵌套时保留描边作区分，`.grouped` 嵌套 `.grouped` 无视觉区分（已知限制）
- 内边距：默认 `CoreSpacing.lg`
- 圆角 / 描边：由 `SurfaceModifier` 统一提供，不在 `Card` 自身重复定义；`.content` 在顶层有描边、嵌套（elevated）时 iOS 无描边 / macOS 仍描边，`.grouped` 无描边
