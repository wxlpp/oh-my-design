...
| 切换动画 | UIKit 自己的 | `CoreMotion.selection`（`.snappy`，0.22 s）+ `matchedGeometryEffect`；Reduce Motion 下滑块不滑，旧位置淡出、新位置淡入（#407） |
...
1. **判不了时长**：`easeInOut` 尾巴很长，末段每帧只挪 1 px，拿它对 `duration: 0.18` 的账会得到不稳的数（本节录屏取于 #407 之前；#407 起曲线是 `CoreMotion.selection`）。
