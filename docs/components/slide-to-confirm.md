...
- **iOS 上只认领横向占主导的滑动**（`SlideToConfirmPanArbitration.claims`：|dx| > |dy|，按下点起算）：
  ...
  ⚠️ **起手斜向（|dy| ≥ |dx|，含正斜 45°）的触摸不会被认领，即使外面没有任何滚动容器**：
  这样的触摸不开会话、不吸收，交给指示器下面的任何东西（滚动视图、系统返回手势，或者什么都不发生）。
- **只认第一根手指**：识别器 `maximumNumberOfTouches = 1`，`shouldReceive` 只在识别器空闲（`.possible` 且
  尚未跟踪任何触摸）时收下触摸并记按下点，其余触摸一律不收（`SlideToConfirmPanArbitration.admits`）。
  不这样做，第二根手指落下会改写按下点、位置取多指质心而跳变；更糟的是「手指 1 在指示器上按下 →
  手指 2 落在轨道别处 → 抬起手指 1 → 手指 2 滑满」会沿用手指 1 的起点，绕过「起点须在指示器上」。
  按下点在 `.ended` / `.cancelled` / `.failed` 时清空。
- **按下点存窗口坐标，动作时经 `converter.convert(globalPoint:to: .local)` 换回控件本地空间**，
  位移 = 本地当前位置 − 本地按下点。这样 `scaleEffect` 等变换下位移与全程同一量纲（见下方实测）；
  认领裁决只比方向，直接在窗口坐标里算。
- 识别器状态到 core 事件的映射是纯函数 `SlideToConfirmPanArbitration.event(phase:movement:localX:)`：
  `began` / `changed` ⇒ 拖动（起点 = 当前位置 − 位移），`ended` ⇒ 松手，`cancelled` / `failed` ⇒ **只打断**
  （被取消的滑满绝不按松手判定），其他状态不产事件。iOS 侧只做 `UIGestureRecognizer.State` → phase 的换算。
- **横向滚动容器**（`ScrollView(.horizontal)`、`TabView` 的 `.page` 样式）：轨道上起手的横滑一律归本控件——
  从指示器滑满照常触发、外层不动；从轨道空白处横滑被吸收，外层**不滚动 / 不翻页**。
  外层要从轨道之外起手才能滚动 / 翻页（见下方实测）。这与「整条轨道吸收横滑」的定案一致，
  没有加 `require(toFail:)` 之类的识别器关系。
...
- **手势认领（纯函数）**：`SlideToConfirmPanArbitration.claims` 横向严格占主导才认领；纯纵向、正斜 45°、零位移都不认领。
  `admits` 只在识别器空闲且未跟踪任何触摸时收新触摸。`event(phase:movement:localX:)`：`began` / `changed` 的起点
  = 当前位置 − 位移，`ended` 给松手，`cancelled` / `failed` 只给打断、绝不给松手，其他状态不产事件。
  core 另有一条：有效会话被打断后，下一段起点不在指示器上的拖动重新裁决为只吸收。
  iOS 识别器经这些函数裁决、位移从按下点算、按下点换回本地空间，这几处只有源码接线判据（下方「视图接线」）。
...
- **视图接线（源码级）**：iOS 平移识别器的几处（`shouldReceive` 经 `admits` 才记按下点、`maximumNumberOfTouches = 1`、
  `gestureRecognizerShouldBegin` 经 `claims`、按下点经 `converter` 换回本地再算位移、状态经 `event` 映射、
  序列结束清空按下点、状态 → phase 的换算、`isEnabled` 随宿主）与它转交编排器的三路；macOS `@GestureState` + `updating`、带起点与方向系数的 `onChanged`、
...
只收第一根手指、按下点换回本地空间之后的一轮（iOS 26.4 模拟器，专用设备；画廊页临时加了三个探针——
`.scaleEffect(0.8)` 的实例、`ScrollView(.horizontal)` 里宽 300 的实例、`TabView(.page)` 里的实例，实测后已还原）：

| 操作 | 读数 |
|---|---|
| 回归：滑到底（54 → 395）/ 阈值前一点（54 → 336）/ 甩一半 / 反向 / 轨道空白处右滑 | `Confirmed 1` / 不变 / 不变 / 不变 / 页面未 pop |
| 回归：轨道空白处纵向上滑 / 下滑 | 页面滚动（按钮 y 351.7 → −187.7 → 351.7） |
| `scaleEffect(0.8)`（屏上轨道宽 276.8 ⇒ 屏上全程 235.2 pt，指示器中心 x = 83）：83 → 310（屏上 227 ⇒ 本地 283.75） | 不触发 |
| 同上：83 → 322（屏上 239 ⇒ 本地 298.75）/ 100 → 342（本地起点 46.75，在命中区内） | 各触发一次 |
| 同上：100 → 330（本地 287.5）/ 起点 107（本地 55.5，出了命中区）→ 399 | 都不触发 |
| 对照，修前构建（按下点在识别器视图坐标、位移按屏上点数）：83 → 322 / 100 → 342 | **都不触发**——屏上滑到尽头不够 |
| 对照，修前构建：83 → 380（屏上 297，越过轨道可见尽头 41 pt） | 才触发 |
| RTL（直达预览）：右滑指示器 / 空白处左滑 / 从右端指示器左滑 300（< 318）/ 左滑满 | `0` / `0` / `0` / `Confirmed 1` |
| RTL + `scaleEffect(0.8)`：328 → 90（本地 297.5 < 318）/ 328 → 68（本地 325） | 不触发 / 触发 |
| `ScrollView(.horizontal)`：从指示器滑满 | 触发，外层偏移仍为 0 |
| 同上：轨道空白处左滑（250 → 30 慢、300 → 30 与 200 → 20 快）；从指示器左滑 | 外层偏移都为 0，不触发 |
| 对照：同一 `ScrollView` 里轨道右侧（x = 365）起手左滑 | 外层偏移 362 |
| `TabView(.page)`：从指示器滑满（慢 / 快各一次） | 各触发一次，仍在第 0 页 |
| 同上：轨道空白处左滑（慢 1 次、快 2 次）；从指示器左滑 | 都停在第 0 页，不触发 |
| 对照：同一 `TabView` 里轨道上方（y = 440）起手左滑 | 翻到第 1 页 |
| 宿主 `.disabled` 的那一行：从指示器（x = 54）右滑、从空白处（x = 150）右滑 | 页面**都被 pop** |
| 对照：同一时刻可用行空白处（x = 150）右滑 | 页面未 pop |

多指（第二根手指改写按下点、抬起先落的手指后绕过起点裁决）**没有真实触摸实测**：axe 只发单指触摸，
这一条只有 `admits` 的纯函数判据与源码接线判据。
...
- **宿主 `.disabled` 时横滑会漏给返回手势**：手势随宿主禁用而停用，实测在导航栈页面里从禁用行的
  指示器或空白处向右横滑，页面被 pop（`DragGesture` 构建与平移识别器构建各测过，结果相同）。
  action 不会被触发（见上方直达预览那一轮）。
- **横向滚动容器里，轨道这一段拖不动外层**：见上方《三个阶段与可交互性》，外层须从轨道之外起手。
