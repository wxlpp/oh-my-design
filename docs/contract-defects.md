...
- **不是扩展点**：A–D 讲的都是把定制权**交给调用方**的形态（协议让第三方写实现，槽让调用方替换一处视觉，
  ...
  差异逐项落在 `#422` 步骤 2 的装饰档原文「连线 / 选中块 / 尺寸 / 缩进引导线」里——步骤 2 已判它们是皮肤变体、
  不计入 ≥2。⇒ 这是「规定性组件自带的装饰选项」，步骤 3 的结论不受影响。
- **是扩展点**：从调用方看，它与形态 D2（公开配置枚举，调用方在封闭集合里选外观）同形，只是载体是
  `struct` + 静态成员而不是 `enum`；且它一经发布就删不掉（与 `D-312-1` 登记的「已发布的 D2 枚举不受祖父条款保护」
  同一个问题）。

**灰区子问题（悬停高亮）**：两外观差异里只有悬停高亮**不在** `#422` 装饰档原文里。按补充规则 1 论证：
它是背景层；不进任何绑定、不进无障碍树、不改变可见行 / 选中 / 焦点 / 展开任一状态——倾向装饰。
**但**它随「指针在本行」这一交互状态变化；补充规则 1 的反例（`SidebarStatusFooter` 的状态圆点，颜色即状态）
判的是承载**组件状态**的层，悬停承载的是指针位置这一**宿主输入**状态，公约没有区分这两者 ⇒ 落在灰区。
...
**本轮处置**：只登记，**不改判**（`Tree` 的 `kind` / `decidedBy` / `needsExtensionPoint` 不动，不走修订回路）；
读法与灰区连同本条待公约 owner 裁定。为保住「以后改判也不必推翻公开 API」，`TreeStyle` 刻意做成封闭 `struct`
